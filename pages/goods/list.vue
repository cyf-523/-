<template>
	<view class="">
		...
		<u-row gutter="16" class="u-skeleton" style="padding-left: 10rpx;padding-right: 10rpx;">
			<u-col span="6" v-for="(good, index) in productList.length !==0 ? productList : [{skuInfo:{}}, {skuInfo:{}}, {skuInfo:{}}, {skuInfo:{}}]" :key="good.skuInfo.id">
				<goods v-show="productList.length != 0" :good="good"></goods>
			</u-col>
		</u-row>
		<u-empty text="数据为空" mode="data" v-show="productList.length == 0"></u-empty>
		<u-divider bg-color="#F5F5F5" :height='60' border-color='#888' v-show="productList.length != 0">到底了</u-divider>
		<!-- 骨架屏 -->
	...
	export default {
		...
		methods: {
			...
			async getProductList() {
				// 请求数据之前显示骨架屏
				this.loading = true;
				let result = await this.$u.api.reqProductList(this.params);
				// console.log(result)
				// 隐藏骨架屏
				this.loading = false
				if(result.code == 200) {
